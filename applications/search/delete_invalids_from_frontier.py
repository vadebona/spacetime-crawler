...
@Deleter(VadebonaAdesanyoKdmontenLink)
@Getter(VadebonaAdesanyoKdmontenLink)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(VadebonaAdesanyoKdmontenLink)
        print "Found ", len(ls), " links."
        count = 0
        for l in ls:
            if not is_valid(l.full_url):
                self.frame.delete(VadebonaAdesanyoKdmontenLink, l)
                count += 1
...
class Simulation(object):
    ...
    def __init__(self, address, port):
        ...
        objectless_connector = ObjectlessSpacetimeConnection(
            "ResetFrontier_VadebonaAdesanyoKdmonten".format(CrawlerFrame.app_id),
